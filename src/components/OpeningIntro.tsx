...
export default function OpeningIntro({
  ...
  linkText = "www.grazi-portfolio.vercel.app",
}: OpeningIntroProps) {
    ...
    <AnimatePresence>
      {visible && (
        ...
        >
          ...
          <motion.div
            variants={bgGlow}
            className="pointer-events-none absolute inset-0 bg-[radial-gradient(100%_60%_at_8%_0%,rgba(139,92,246,.25),transparent_40%),radial-gradient(90%_70%_at_100%_20%,rgba(99,102,241,.2),transparent_40%)]"
          />
          <motion.div
            variants={bgGlow}
          ...
          <div className="relative grid h-full place-items-center px-4">
            <motion.div className="w-full max-w-5xl" variants={container}>
              ...
              <div className="text-center">
                ...
                <motion.h2
                  variants={fadeUpSoft}
                  className="mt-2 text-4xl sm:text-6xl md:text-7xl font-extrabold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-indigo-300 via-violet-300 to-indigo-400"
                >
                  {headlineBottom}
                </motion.h2>
              </div>
          ...
          <button
            onClick={close}
            className="absolute top-6 right-6 rounded-full border border-white/15 bg-indigo-400/20 px-3 py-1.5 text-xs text-indigo-600 dark:text-indigo-300 font-light hover:bg-white/10 transition"
          >
            Pular intro
          </button>
        </motion.div>
